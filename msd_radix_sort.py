'''def radix_sort_MSD_for_strings(array, i):
    ...
    # if list has only one element it returns the only element
    if len(array) <= 1:
        return array
...
def main():
    array_of_strings = ['VEYSEL','EGE','YASIN', 'SELIN']# ASCII TABLE A-Z is from 64 to 90 if you want to change it to lowercase letter 
                                                        # Also change the bucket list's range                
